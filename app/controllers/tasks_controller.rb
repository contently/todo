class TasksController < ApplicationController
  ...
  # GET /tasks.json
  def index
    # HW4: only display the tasks we need
  ...
  private
    ...
    # HW4: we'll need to know which list we're working with
    def set_list
      @list = List.where('id = ? AND user_id = ?', params[:list_id], @current_user.id).first
    end
